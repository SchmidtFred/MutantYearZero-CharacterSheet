...
export default function ChooseTalentsAndMutations(props) {
	...
	const handleTalentChange = (event) => {
		setChosenTalent(
			startingTalents.find((t) => t.id === event.target.value)
		);
	};
	...
	const handleSecondMutation = () => {
		if (chosenReducedAttribute === "") {
			window.alert(
				"You must choose an attribute to reduce before drawing a second mutation."
			);
		} else {
	...
	const handleChosenAttributeChange = (event) => {
		...
		if (event.target.value === chosenReducedAttribute) {
			window.alert(
				"You have somehow set an attribute to be reduced twice. Please avoid doing that again. We have stopped it from happening."
			);
		} else {
			let canReduce = true;
			...
			if (chosenReducedAttribute !== "") {
				switch (chosenReducedAttribute) {
					...
					default:
						throw new Error(
							"Something went wrong with UnReducing chosen attribute"
						);
						break;
				}
			}
			...
			if (canReduce) {
				...
				switch (event.target.value) {
					...
					default:
						throw new Error(
							"Something went wrong with Reducing chosen attribute"
						);
						break;
				}
				...
				window.alert(
					"The attribute you are currently reducing is not able to be brought to a higher stat than it currently is at. Please reduce it by one on the stats menu before switching off of it here."
				);
			}
		}
	};
	...
	const SecondMutationDialog = ({ open }) => {
		return (
			<Dialog open={open} onClose={() => setMutOpen(false)}>
				<DialogTitle textAlign='center' pt={2} pb={2}>Second Mutation</DialogTitle>
				<Typography variant="body1" p={1}>
					This is a permanent change and CANNOT be undone. Once you
					have decided your character has a second mutation, they have
					it for life. Think carefully before confirming.
				</Typography>
				<ButtonGroup
					size="large"
					orientation="horizontal"
					variant="contained"
                    sx={{width: '30%', margin: 'auto', mb: 1, mt: 1}}
				>
					<Button
						onClick={() => {
							handleSecondMutation();
	...
	const RedrawMutationDialog = ({ isSecondMutation, onClose, open }) => {
		return (
			<Dialog onClose={() => onClose(!secondMutation, false)} open={open}>
				...
				<Typography variant="body1" p={1}>
					You don't get to choose your mutation. You must learn to
					make the most of the mutation you've got, even if you would
					have preferred another. No one chooses what they are born
					with.
				</Typography>
				<Typography variant="body1" p={1}>
					However if another party member, or yourself, has drawn this
					same mutation you may redraw a new one to avoid duplicates.
				</Typography>
				<ButtonGroup
					size="large"
					orientation="horizontal"
				...
				>
					<Button
						onClick={() => handleClose(!isSecondMutation, true)}
					>
						Draw
					</Button>
					<Button
						onClick={() => handleClose(!isSecondMutation, false)}
					>
						Cancel
					</Button>
				</ButtonGroup>
					...
					>
						...
						<Grid item>
							<Typography variant="body1" sx={{ flex: 1 }}>
								{chosenTalent.description
									? `See more details at Pg. ${chosenTalent.description} of the core rulebook.`
									: "Select your starting talent."}
							</Typography>
						</Grid>
					</Grid>
				</Grid>

				<Divider orientation="vertical" flexItem sx={{backgroundColor: theme.palette.primary.dark}} />

				<Grid item container sx={{ flex: 1 }} direction="column" spacing={0}>
					<Grid item textAlign="center" mb={2}>
						<Typography
							variant="h5"
							width="100%"
							...
							<>
								...
								>
									...
									>
										...
										<Grid item>
											<Typography variant="body1">
												For details, refer to Pg.{" "}
												{mutation.description} in the
												core rulebook
											</Typography>
										</Grid>
									</Grid>
								...
								<Divider orientation="horizontal" flexItem sx={{backgroundColor: theme.palette.primary.dark}} />
										...
										>
											<Grid item textAlign="center">
												...
												>
													...
													first mutation, you can draw
													a second mutation and start
													the game with two. The price
													you pay is that you must
													decrease on of your
													attribute scores by one
													step.
												</Typography>
												<Typography variant="body2" p={2}>
													If you choose to have two
													mutations, you may end up
													starting the game with the
													score of 1 in an attribute.
													More info found on Pg. 19 of
													the core rulebook.
												</Typography>
											</Grid>
											<Grid
